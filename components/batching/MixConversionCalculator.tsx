...
export default function MixConversionCalculator({
  ...
  const [targetMixId, setTargetMixId] = useState<string>(() => {
    if (mixDesigns.length > 1) return mixDesigns[1].id;
    return mixDesigns.length > 0 ? mixDesigns[0].id : "";
  });
  ...
  const sourceMix = useMemo(() => {
    return mixDesigns.find((m) => m.id === sourceMixId) || mixDesigns[0] || null;
  }, [mixDesigns, sourceMixId]);

  const targetMix = useMemo(() => {
    return mixDesigns.find((m) => m.id === targetMixId) || mixDesigns[0] || null;
  }, [mixDesigns, targetMixId]);
...
• Plasticizer: ${d.plasticizerFlOz} fl oz
• Retarder: ${d.retarderFlOz} fl oz
---------------------------------
    ...
    <div style={{ maxWidth: "1200px", margin: "0 auto", paddingBottom: "80px" }}>
      ...
      >
              ...
              <div style={{ display: "flex", gap: "4px" }}>
                {[3, 4, 5, 8, 9, 10].map((v) => (
                  <button
                    ...
                    onClick={() => {
                      setSourceVolume(v);
                      if (targetVolume < v) setTargetVolume(v);
                    }}
                    style={{
                      background: sourceVolume === v ? "rgba(239, 68, 68, 0.2)" : "rgba(255,255,255,0.05)",
                      border: sourceVolume === v ? "1px solid #ef4444" : "1px solid var(--border)",
                      color: sourceVolume === v ? "#f87171" : "var(--text-secondary)",
                      borderRadius: "6px",
                      padding: "6px 8px",
                      fontSize: "0.8rem",
                      fontWeight: "700",
        ...
        >
          ...
          <span style={{ fontSize: "0.75rem", fontWeight: "800", color: "#60a5fa", textAlign: "center" }}>
            {conversionResult ? `+${conversionResult.addedVolume} yd³` : "CONVERT"}
          </span>
        </div>
        ...
        >
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              ...
              />
              <div style={{ display: "flex", gap: "4px" }}>
                {[sourceVolume, 8, 9, 10, 11].map((v) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => setTargetVolume(v)}
                    style={{
                      background: targetVolume === v ? "rgba(34, 197, 94, 0.2)" : "rgba(255,255,255,0.05)",
                      border: targetVolume === v ? "1px solid #22c55e" : "1px solid var(--border)",
                      color: targetVolume === v ? "#4ade80" : "var(--text-secondary)",
                      borderRadius: "6px",
                      padding: "6px 8px",
                      fontSize: "0.8rem",
                      fontWeight: "700",
